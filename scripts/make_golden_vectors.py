...
def build_clips(regenerate_decoded: bool = False) -> dict[str, np.ndarray]:
    """Assemble the fixture set.

    The synthetic clips are pure arithmetic and are regenerated every run —
    they were verified bit-identical across numpy 2.5 and 1.26.

    The real clip is DIFFERENT and is not regenerated by default. It comes from
    decoding an mp4, and ``cv2.resize``/decode differ between OpenCV builds:
    the committed fixture's sha changed when the environment moved from OpenCV
    5.0.0 to the pinned 4.8.1.78. A fixture that changes with the decoder is
    not a fixture, and every reference embedding built against it silently
    stops corresponding to it.

    So the committed ``.npz`` of DECODED FRAMES is the source of truth, and
    ``regenerate_decoded=True`` is the only way to re-derive it from video.
    """
    ...
    committed = CLIP_DIR / "real_test_video.npz"
    if not regenerate_decoded and committed.exists():
        with np.load(committed) as data:
            clips["real_test_video"] = np.asarray(data["clip"])
        return clips
...
def decoding_stack() -> dict[str, str]:
    """The stack that decoded the real clip, recorded with the fixture.

    A decoded-frame fixture is only reproducible on the build that decoded it.
    Recording the stack turns "why did this sha change?" from an investigation
    into a diff.
    """
    import sys as _sys

    versions = {"python": ".".join(str(v) for v in _sys.version_info[:3])}
    try:
        import cv2
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "re-derive the real clip from video. Off by default: decoding "
            "differs between OpenCV builds, so this changes the fixture sha "
            "and invalidates every reference built against it."
        ),
    )
    args = parser.parse_args(argv)

    print("Building golden input clips...")
    clips = build_clips(args.regenerate_decoded)
    ...
    if args.clips_only:
        # Preserve any references already recorded. Blanking them would make a
        # clips-only run look like a run that produced no references, which is
        # a different and much worse state.
        existing: dict[str, Any] = {}
        if MANIFEST_PATH.exists():
            existing = json.loads(MANIFEST_PATH.read_text()).get("references", {})
        write_manifest(clip_shas, existing or None, ["--clips-only requested"])
